...
public class GoalRevealFromBox : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (glowSpriteRenderer == null)
        {
            Transform glowTransform =
                transform.Find("HeartGlow");

            if (glowTransform != null)
            {
                glowSpriteRenderer =
                    glowTransform.GetComponent<SpriteRenderer>();
            }
        }
    ...
    private IEnumerator RevealRoutine()
    {
        ...
        Vector3 popPos =
            GetResolvedPopPosition();

        Vector3 finalLandPos =
            GetResolvedLandPosition();
        ...
        // Этап 2: падение к HeartLandPoint
        if (useLandPoint &&
            landPoint != null)
        {
            float landTime = 0f;

            Vector3 landStartPos =
                popPos;
        ...
        {
            Debug.Log(
                "[GoalRevealFromBox] Goal reveal finished.",
                this
            );
        }
    }
    ...
    private Vector3 GetResolvedPopPosition()
    {
        Vector3 result =
            popPoint != null
                ? popPoint.localPosition
                : startLocalPosition;

        if (mirrorByPlayerSide &&
            IsPlayerOnRight())
        {
            result.x =
                -result.x;
    ...
    private bool IsPlayerOnRight()
    {
        ...
        if (player == null)
        {
            ...
            {
                Debug.LogWarning(
                    "[GoalRevealFromBox] Player not found by tag: " +
                    playerTag,
                    this
                );
            }
